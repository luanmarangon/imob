...
<section class="dash_content_app">
    <header class="dash_content_app_header">
        <h2 class="icon-user-plus">Proprietários</h2>
        <form action="<?= url("/admin/owners/owners"); ?>" method="post" class="app_search_form">
            <input type="text" name="s" value="<?= $search; ?>" placeholder="Pesquisar Proprietário:">
        ...
        <section class="app_control_plans">
            <?php foreach ($owners as $owner) : ?>
                <article class="radius">
                    <div>
                        <h4 class="icon-user"><?= $owner->fullName(); ?></h4>
                        <!-- <?= var_dump($ownersContacts); ?></p> -->
                        <?php foreach ($ownersContacts as $contacts) : ?>
                            <?php if ($contacts->owners_id === $owner->id) : ?>
                    ...
                    <div class="actions">
                        ...
                        <a class="icon-phone btn btn-green" href="<?= url("admin/owners/owners/{$owner->id}/contacts"); ?>" title="">Contatos</a>
